...
M=[     L1,      -L_k,   L3,   L0;
 ...
 theta(i)=(sum(abs(U).^2)*dx-sum(abs(V).^2)*dx)/(sum(abs(U).^2)*dx+sum(abs(V).^2)*dx);
 i=i+1;
end
hold on
box on
plot(P(1:6),abs(theta(1:6)))
plot(P(7:16),abs(theta(7:16)),'--r')
plot(P(17:40),abs(theta(17:40)))
plot(P(41:51),abs(theta(41:51)),'--r')
plot(P(51:end),abs(theta(51:end)))
% plot(mu,P,'--r','LineWidth',1.5)
% plot(mu(1:end),P(1:end),'-.o','LineWidth',1.5)

% save('E:\dual-core\SSB_j\data\0w0=0_l=2_k=0.3_mu_p_asy.mat','P','theta','lambda','mu')
